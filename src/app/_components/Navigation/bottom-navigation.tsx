...
export default function BottomNav({ children, session, userAccesses }: { children: ReactNode, session: Session | null, userAccesses: IUserAccessResponse[] }) {
        ...
        <Box sx={{ pb: 7, pt: 0 }}>
            ...
            <Paper sx={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 1071 }} elevation={3}>
                ...
                >
                    ...
                    {session && showConsumption && <BottomNavigationAction sx={{ minWidth: "60px" }} label="Kulutus" href="/consumptions" value="/consumptions" icon={<ElectricBoltIcon />} LinkComponent={Link} />}
                    {session && showProduction && <BottomNavigationAction sx={{ minWidth: "60px" }} label="Tuotto" href="/productions" value="/productions" icon={<SolarPowerIcon />} LinkComponent={Link} />}
