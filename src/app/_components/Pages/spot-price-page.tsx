...
import {Button, Col, Radio, type RadioChangeEvent, Row, Space, Table} from "antd";
const {Column} = Table;
    ...
    const filters = () => {
        switch (timePeriod) {
            ...
            case TimePeriod.PT1H:
                return (
                    <Row style={{paddingBottom: 8}}>
                        <Col flex="auto"><DayDatePicker value={startDate} onChange={onDateChange}
                                                        disabledNextDays={1}></DayDatePicker></Col>
                        {showUpdateButton() && (
                            <Col flex="none">{(<Button loading={isUpdating} onClick={executeUpdateSpotPrices}
                                                       icon={!isUpdating && <RedoOutlined/>}></Button>)}</Col>
                        )}
            ...
            case TimePeriod.P1D:
                return (
                    <Row style={{paddingBottom: 8}}>
                        <Col flex="auto"><MonthDatePicker value={startDate}
                                                          onChange={onDateChange}></MonthDatePicker></Col>
                    </Row>
                )
            case TimePeriod.P1M:
                return (
                    <Row style={{paddingBottom: 8}}>
                        <Col flex="auto"><YearDatePicker value={startDate}
                                                         onChange={onDateChange}></YearDatePicker></Col>
                    </Row>
                )
            case TimePeriod.P1Y:
                return (
                    <Row style={{paddingBottom: 8}}>
                        <Col flex="auto"><YearRangeDatePicker startYear={startDate} endYear={endDate}
                                                              onChange={onDateRangeChange}/></Col>
                    </Row>
                )
            default:
                return (<></>)
    ...
    return (
        <Space direction="vertical" className="text-center" style={{width: 'calc(100vw - 32px)'}}>
            ...
            <Table
                ...
                pagination={false}
            >
                <Column title="" dataIndex="time" key="current" width={30} render={(data: string | number | Date) => (
                    <>{isCurrentTimePeriod(data) && <CaretRightFilled/>}</>
                )}/>
                <Column title="Aika" dataIndex="time" key="time"
                        render={(data: Dayjs) => dateToSpotTimeString(data, timePeriod)}/>
                <Column title={electricityPriceTitle()} dataIndex="price" key="electricity_price"
                        render={(_data: number, row: ISpotPrice) =>
                            ElectricitySpotPrice({spotPrice: row})}/>
                {(timePeriod === TimePeriod.PT15M || timePeriod === TimePeriod.PT1H) && (
                    <Column title="Sähkön hinta" dataIndex="price" key="full_price"
                            render={(_data: number, row: ISpotPrice) =>
            ...
            {session && (
                <Row>
                    {<Col xs={24} style={{textAlign: 'right'}}>{(
                        <Button loading={isUpdating} onClick={executeUpdateSpotPrices}>Hae hinnat
                            uudelleen</Button>)}</Col>}
                </Row>
            )}
        </Space>
    );
