...
export default function WattivahtiProductionPage({
  timePeriod,
}: IWattivahtiProductionPageProps) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_data: number, row: IWattiVahtiProduction) =>
        WattiVahtiProduction({ production: row, timePeriod }),
    },
    {
      title: "Arvo",
      dataIndex: "price",
      key: "price",
      render: (_data: number, row: IWattiVahtiProduction) =>
        WattiVahtiProductionPrice({ production: row, timePeriod }),
    },
  ];
  ...
  const missingValues = () => {
    ...
    if (
      (timePeriod === TimePeriod.PT15M &&
        (productions.length ?? 0) < hoursInDay() * 4) ||
      (timePeriod === TimePeriod.PT1H &&
        (productions.length ?? 0) < hoursInDay())
    ) {
      return (
        <AlertWarning
          title="Huom!"
          message="Kaikkia päivän tuottoja ei ole saatavilla."
    ...
    if (
      ...
      (productions.length ?? 0) < daysInMonth()
    ) {
      return (
        <AlertWarning
          title="Huom!"
          message="Kaikkia kuukauden tuottoja ei ole saatavilla."
          type="borderedWithAccent"
        />
      );
    }
  ...
  return (
    ...
    >
      ...
      <WattiVahtiProductionSummary
        timePeriod={timePeriod}
        summary={response?.summary}
        isLoading={isLoading}
      />
      <WattiVahtiProductionsChart
        wattivahtiResponse={response}
        startDate={startDate}
        isLoading={isLoading}
        endDate={endDate}
      />
